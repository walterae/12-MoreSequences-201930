...
def run_test_generate_points_on_circle():
    ...
    expected = [rg.Point(215, 150),  # All numbers are approximate.
                rg.Point(200, 165),
                rg.Point(185, 150.0),
                rg.Point(200, 135.0)]
    circle = rg.Circle(rg.Point(200, 150), 15)
...
def run_test_draw_points_on_circle():
    ...
    window = rg.RoseWindow(310, 430, title)
    circle = rg.Circle(rg.Point(150, 120), 80)
...
def run_test_pizza():
    ...
    title = 'PIZZA test 4:  6 yellow slices, thin (thickness=3) black lines.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 150)
    circle.fill_color = 'yellow'
    circle.outline_thickness = 3
    pizza(window, circle, 6, 'black', 3)
    window.close_on_mouse_click()
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    for k in range(len(lines)):
        line = rg.Line(circle.center,lines[k])
        line.color = color
...
def run_test_polygon():
    ...
    title = ('POLYGON tests 3:  8 segments with thick red lines;'
             + ' 6 with medium red lines.')
    window = rg.RoseWindow(550, 400, title)
...
def run_test_fancy_polygon():
    ...
    circle = rg.Circle(rg.Point(250, 165), 180)
    circle.fill_color = 'black'
    fancy_polygon(window, circle, 300, 7, 'red', 61)
    window.close_on_mouse_click()
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color,
    ...
    for k in range(len(points)):
        ...
        line = rg.Line(points[k],points[(((k+ hops_to_next_point) % number_of_lines) + number_of_lines) % number_of_lines])
        line.color = color
        line.thickness = thickness
        line.attach_to(window)
    window.render()
